...
<body>
    ...
    function gen_data_blog($row)
    {
        ...
        echo '<h6 class="blog-post-title">' . $row['jour_study'] . '</h6>';
        $sql = "SELECT * FROM co_worker WHERE cow_ref_table = 'journal' AND cow_ref_id = " . $row['jour_id'] . " ORDER BY cow_ratio DESC";
        ...
        echo $row['jour_owner_prename'] . $row['jour_owner_firstname'] . ' ' . $row['jour_owner_lastname'] . $researcher_list;
        echo '</p>';
        echo '<p class="research_source"><strong>วารสาร</strong> ' . $row['jour_journal'] . '</p>';
        echo '<p class="blog-post-meta"><strong>ระยะเวลา</strong> ';
        echo $fnc->gen_date_range_semi_th($row["jour_date_available"]);
        echo '</p>';
        echo '</div>';
    }
    ?>
        ...
        <div class="row">
            <div class="col-md-8">
                <h2 class="pb-4 mb-4 blog-post-title border-bottom" style="font-size: 2rem;">
                    ผลงานตีพิมพ์ ปีงบประมาณ <?=$fyear;?>
                </h2>

                <?php
                $sql = "SELECT * FROM journal WHERE jour_status = 'enable' AND jour_fiscalyear = '" . $fyear . "' ORDER BY jour_fiscalyear DESC";
                ...
                <nav class="blog-pagination">
                    <?php
                    $data_count = 0;
                    $sql = "SELECT COUNT(jour_id) AS Count_data FROM research WHERE jour_status = 'enable' AND jour_fiscalyear = '" . $fyear . "'";
            ...
            <aside class="col-md-4">
                ...
                <div class="p-4">
                    ...
                    <ul class="mb-0">
                    <?php
                        $sql = "SELECT jour_fiscalyear FROM journal WHERE jour_status = 'enable' GROUP BY jour_fiscalyear ORDER BY jour_fiscalyear DESC";
                        $fiscalyear = $fnc->get_db_rows($sql);
                        foreach ($fiscalyear as $row) {
                            echo '<li><a href="?fyear=' . $row['jour_fiscalyear'] . '" class="news_link">ปี งปม. ' . $row['jour_fiscalyear'] . '</a></li>';
                        }
                        ?>
                    </ul>
                </div>
